...
<body class="bg-background text-on-background min-h-screen flex flex-col justify-center items-center px-6 py-12 relative overflow-x-hidden">
  ...
  <!-- Main Container -->
  <div class="w-full max-w-md glass-panel rounded-3xl shadow-xl p-8 relative overflow-hidden">
    ...
    <div class="relative z-10 flex flex-col items-center">
      ...
      @if ($found && $saving)
        ...
        <h2 class="text-xs text-on-surface-variant font-semibold">Nomor Sertifikat: <span class="font-mono text-primary font-bold">{{ $saving->cert_number }}</span></h2>
        ...
        <div class="w-full space-y-4 text-left">
          <div class="bg-surface-container-low p-4 rounded-2xl border border-outline-variant/10 flex flex-col gap-1">
            ...
            <p class="font-serif text-lg font-extrabold text-on-surface italic">{{ $saving->user ? $saving->user->name : 'N/A' }}</p>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="bg-surface-container-low p-3.5 rounded-2xl border border-outline-variant/10">
              ...
              <span class="font-bold text-xs text-on-surface mt-1 block">🐄 {{ $saving->package_name }}</span>
            </div>
            <div class="bg-surface-container-low p-3.5 rounded-2xl border border-outline-variant/10">
          ...
          <div class="bg-surface-container-low p-4 rounded-2xl border border-outline-variant/10 flex items-start gap-3">
            ...
            <div>
              ...
              <p class="font-bold text-xs text-on-surface mt-0.5">{{ $saving->penyaluran_address ?: 'Daerah Minoritas/Darurat' }}</p>
              <span class="text-[9px] text-on-surface-variant mt-0.5 block">Metode: {{ $saving->penyaluran_method ?: 'Titip Qurban' }}</span>
            </div>
          </div>
          ...
          <div class="bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100 flex items-center justify-between">
            ...
            <span class="text-xs font-bold bg-emerald-100 text-emerald-900 px-3 py-1 rounded-full border border-emerald-200">{{ $saving->penyaluran_status ?: 'Selesai' }}</span>
          </div>
        </div>
      ...
      @else
        ...
        <h2 class="text-xs text-on-surface-variant font-medium mt-2 text-center max-w-[280px]">Maaf, nomor sertifikat qurban <span class="font-mono text-red-600 font-bold">{{ $cert }}</span> tidak terdaftar dalam database kami.</h2>
      ...
      <a href="/" class="w-full bg-primary text-on-primary font-bold py-3.5 rounded-2xl shadow-md hover:bg-primary/95 transition-all text-xs flex items-center justify-center gap-1.5 mt-8">
